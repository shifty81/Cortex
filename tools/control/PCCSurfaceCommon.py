...
class BackendClient:
    ...
    def _resolve_provider_script(self) -> Path:
        ...
        if declared:
            ...
            try:
                declared_path.relative_to(self.root)
            except ValueError as exc:
                raise SurfaceError(
                    "Declared machine provider escapes the selected project root: "
                    f"{declared_path}"
                ) from exc
...
def _provider_parser_commands(root: Path) -> set[str]:
    """Read Cortex argparse choices without importing/executing project code."""
...
def validate_surface(root: Path) -> list[str]:
    contract = ProjectContract.load(root)
    valid_risks = {"read_only", "local_mutation", "external_mutation", "destructive"}
    invalid = sorted({item.risk for item in contract.commands if item.risk not in valid_risks})
    if invalid:
        raise SurfaceError(
            "Project command contract contains unsupported risk value(s): " + ", ".join(invalid)
        )
